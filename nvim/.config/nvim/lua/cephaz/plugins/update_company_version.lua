return {
  {
    ...
    config = function()
      local function update_changelog(repos_to_update)
        ...
        local footer = footer_start and content:sub(footer_start)
          or '## 🧪 Testé / déployé\n...'
        ...
        for _, repo_info in ipairs(all_repos) do
          ...
          if is_targeted then
            local current_v = content:match(
              repo_info.label .. ' %- (%d+%.%d+%.%d+)'
            ) or '0.0.0'
            local latest_v_found = nil
            local repo_lines = {}

            local cmd = string.format(
              "git -C ../%s log master --pretty=format:'%%h|%%an|%%s' -n 50 2>/dev/null",
              repo_info.name
            )
            ...
            for i, line in ipairs(git_logs) do
              ...
              if v_from and v_to then
                ...
                if parent_line then
                  local p_hash, p_author_git, p_title =
                    parent_line:match '([^|]+)|([^|]+)|([^|]+)'
                  local pr_num = p_title:match '#(%d+)'

                  -- RÉCUPÉRATION DU PSEUDO (Dictionnaire -> GitHub CLI -> Git Name)
                  local github_handle =
                    get_github_handle(repo_info.name, pr_num, p_author_git)

                  local link = pr_num
                      and (base_url .. repo_info.name .. '/pull/' .. pr_num)
                    or (base_url .. repo_info.name .. '/commit/' .. p_hash)
                  local clean_title =
                    p_title:gsub(' %((#%d+)%)$', ''):gsub(' %(#%d+%)$', '')
        ...
        local full_content = table
          .concat(final_sections, '\n')
          :gsub('\n\n\n', '\n\n') .. '\n' .. footer
        ...
        if f_write then
          ...
          vim.notify(
            '✅ Changements appliqués avec pseudos GitHub',
            vim.log.levels.INFO
          )
        end
      end

...
local function apply_theme(name)
  ...
  if ok then
    vim.notify(('Theme: %s'):format(name))
    return
  end

  vim.notify(('Theme switch failed for "%s": %s'):format(name, err), vim.log.levels.ERROR)
end
...
local function cycle_theme(step)
  ...
  index = ((index - 1 + step) % #theme_cycle) + 1
  apply_theme(theme_cycle[index])
end

local function select_theme()
  vim.ui.select(theme_cycle, { prompt = 'Select theme' }, function(choice)
    if choice then
      apply_theme(choice)
    end
  end)
